...
if (@$_SESSION['registration'] == true) { // в случае уже успешной регистрации будет отображаться это
...
if (isset($_POST['submit'])) {
    ++$_SESSION['counter']; // при нажатии на кнопку submit включается счетчик
    $cookie_name = 'user';
    $cookie_value = 'bla';
    setcookie($cookie_name, $cookie_value, time() + 3600, '/'); // установили cookie на час
    if ($_SESSION['counter'] >= 3) {
        if (isset($_COOKIE[$cookie_name])) { // если куки установлены
            setcookie($cookie_name, $cookie_value, time() + 300, '/'); // поменяй длительность куки на 5 минут
            echo "<h3>Please try again later!</h3>";
        } else {  // при обновлении страницы через 5 минут когда истечет срок куки
            $_SESSION['counter'] = 0; // начни счетчик нажатий кнопки заново
            header('Location: index3.php'); // снова покажи главную странциу с нескрытой формой
        }
...
if ($name_validation && $surname_validation && $email_validation && $password_validation && $confirm_password_validation == true) {
    $_SESSION["name"] = $name;
...
function in_array_r($needle, $haystack, $strict = false) { // ф-ция для поиска в многоуровневом массиве
...
<body>
<div class="registration_form" style="<?= ($_SESSION['counter'] >= 3) ? 'display: none' : ''; ?>">
